...
class JuegosdeportivosCRUDApp:
    def __init__(self, master):
        self.master = master
        self.master.title("CRUD de Juegos Deportivos")
        self.conn = create_connection()
        self.cursor = self.conn.cursor()
        self.create_widgets()
        self.read_juegosdeportivos()  # Cargar juegos al iniciar
    ...
    def create_widgets(self):
        # Crear un marco para los datos de los juegos
        ...
        self.supervisor_label = tk.Label(self.data_frame, text="Supervisor:")
        self.supervisor_label.grid(row=8, column=0, sticky='w')
        self.supervisor_entry = tk.Entry(self.data_frame, validate="key")
        self.supervisor_entry['validatecommand'] = (self.supervisor_entry.register(self.validate_length), '%P', 100) 
        self.supervisor_entry.grid(row=8, column=1)

        self.entidadresponsable_label = tk.Label(self.data_frame, text="Entidad Responsable:")
        self.entidadresponsable_label.grid(row=9, column=0, sticky='w')
        ...
        self.entidadresponsable_entry.grid(row=9, column=1)

        # Botones para CRUD
        # Botón de registrar
        self.create_button = tk.Button(self.data_frame, text="Registrar", command=self.confirm_create_juegodeportivo)
        ...
        self.update_button = tk.Button(self.data_frame, text="Modificar", command=self.confirm_update_juegodeportivo)
        ...
        self.delete_button = tk.Button(self.data_frame, text="Eliminar", command=self.confirm_delete_juegodeportivo)
        self.delete_button.grid(row=10, column=2, pady=10)
        # Inicialmente deshabilitar el botón de eliminar
        self.delete_button.config(state=tk.DISABLED)

        self.read_button = tk.Button(self.data_frame, text="Actualizar lista", command=self.read_juegosdeportivos)
        self.read_button.grid(row=10, column=3, pady=10)

        self.selected_juegodeportivo_id = None  # Inicializar la variable de instancia para el ID
        ...
        # Crear el Treeview
        self.tree = ttk.Treeview(self.tree_frame, columns=("Nombre", "Tema", "Descripción", "Fecha de inicio", "Fecha de Fin", "Área Común", "Duración", "Requisitos", "Supervisor", "Entidad Responsable"), show='headings')
    ...
    def on_double_click(self, event):
        ...
        if selected_item:
            ...
            # Necesitas hacer una consulta para obtener el ID del juego seleccionado
            self.cursor.execute("SELECT IDpractica FROM juegosdeportivos WHERE Nombre = %s", (values[0],))
            self.selected_juegodeportivo_id = self.cursor.fetchone()[0]  # Asigna el ID del juego seleccionado
            print(f"Juego seleccionado ID: {self.selected_juegodeportivo_id}")  # Verifica que sea el ID

            # Resto del código para llenar los campos
            self.nombre_entry.delete(0, tk.END)
            self.nombre_entry.insert(0, values[0])  # Nombre
            self.tema_entry.delete(0, tk.END)
            self.tema_entry.insert(0, values[1])  # tema
            ...
            self.supervisor_entry.insert(0, values[8])  # Requisitos
    ...
    def confirm_create_juegodeportivo(self):
        if messagebox.askyesno("Confirmar", "¿Está seguro que desea registrar este juego?"):
            self.create_juegodeportivo()  # Llama al método original para crear el juego

    def confirm_update_juegodeportivo(self):
        if messagebox.askyesno("Confirmar", "¿Está seguro que desea modificar este juego?"):
            self.update_juegodeportivo()  # Llama al método original para modificar el juego

    def confirm_delete_juegodeportivo(self):
        if not self.tree.selection():
            messagebox.showwarning("Advertencia", "Por favor, selecciona un juego para eliminar.")
            return
        if messagebox.askyesno("Confirmar", "¿Está seguro que desea eliminar este juego deportivo?"):
            self.delete_juegodeportivo()  # Llama al método original para eliminar el juego
     
    def create_juegodeportivo(self):
        ...
        try:
            self.cursor.execute("""INSERT INTO juegosdeportivos (Nombre, Tema, Descripcion, Fechaini, Fechafin, Areacomun, Duracion, Requisitos, Supervisor, Entidadresponsable) 
                                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)""", 
                                (nombre, tema, descripcion, fechaini, fechafin, areacomun, duracion, requisitos, supervisor, entidadresponsable))
            self.conn.commit()

            messagebox.showinfo("Éxito", "Juego registrado exitosamente.")
            self.clear_entries()  # Limpia los campos después de crear el juego
            self.read_juegosdeportivos()  # Actualiza la lista de juegos
        except Exception as e:
            messagebox.showerror("Error", f"Error al registrar el juego: {e}")

            self.read_juegosdeportivos()  # Actualiza la lista de juegos

    def read_juegosdeportivos(self):
        # Limpiar el Treeview
        for row in self.tree.get_children():
            self.tree.delete(row)

        # Leer juegos de la base de datos, incluyendo el ID
        try: 
            self.cursor.execute("SELECT IDpractica, Nombre, Tema, Descripcion, Fechaini, Fechafin, Areacomun, Duracion, Requisitos, Supervisor, Entidadresponsable FROM juegosdeportivos")
            ...
            messagebox.showerror("Error", f"Error al leer los juegos: {e}")

        self.adjust_column_widths()  # Ajusta los anchos de las columnas          

    def update_juegodeportivo(self):
        if self.selected_juegodeportivo_id is None:
            messagebox.showwarning("Advertencia", "Seleccione un juego para modificar")
            return
        ...
        try:
            self.cursor.execute("""
                UPDATE juegosdeportivos 
                SET Nombre = %s, Tema = %s, Descripcion = %s, Fechaini = %s, Fechafin = %s, Areacomun = %s, 
                    Duracion = %s, Requisitos = %s, Supervisor = %s, Entidadresponsable = %s 
                WHERE IDpractica = %s
            """, (nombre, tema, descripcion, fechaini, fechafin, areacomun, duracion, requisitos, supervisor, entidadresponsable, self.selected_juegodeportivo_id))
            
            self.conn.commit()  # Asegúrate de hacer commit

            messagebox.showinfo("Éxito", "Juego actualizado correctamente")
            self.read_juegosdeportivos()  # Actualizar la lista de juegos
        except Exception as e:
            messagebox.showerror("Error", f"No se pudo actualizar el juego: {e}")

    def delete_juegodeportivo(self):
        if self.selected_juegodeportivo_id is None:
            messagebox.showwarning("Advertencia", "Seleccione un juego para eliminar")
            return

        # Confirmar la eliminación
        confirm = messagebox.askyesno("Confirmar", "¿Está seguro de que desea eliminar este juego?")
        if confirm:
            try:
                self.cursor.execute("DELETE FROM juegosdeportivos WHERE IDpractica = %s", (self.selected_juegodeportivo_id,))
                self.conn.commit()
                messagebox.showinfo("Éxito", "Juego eliminado correctamente")
                self.read_juegosdeportivos()  # Actualizar la lista de juegos
                self.selected_juegodeportivo_id = None  # Reiniciar la variable de ID
            except Exception as e:
                messagebox.showerror("Error", f"No se pudo eliminar el juego: {e}")
...
if __name__ == "__main__":
    root = tk.Tk()
    app = JuegosdeportivosCRUDApp(root)
