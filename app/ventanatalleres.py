...
class TalleresCRUDApp:
    def __init__(self, master):
        self.master = master
        self.master.title("CRUD de Talleres")
        ...
        self.read_talleres()  # Cargar talleres al iniciar
    ...
    def create_widgets(self):
        ...
        self.tema_entry['validatecommand'] = (self.tema_entry.register(self.validate_length), '%P', 50)
        ...
        # Botones para CRUD
        # Botón de registrar
        self.create_button = tk.Button(self.data_frame, text="Registrar", command=self.confirm_create_taller)
        ...
        self.update_button = tk.Button(self.data_frame, text="Modificar", command=self.confirm_update_taller)
        ...
        self.delete_button = tk.Button(self.data_frame, text="Eliminar", command=self.confirm_delete_taller)
        self.delete_button.grid(row=10, column=2, pady=10)
        # Inicialmente deshabilitar el botón de eliminar
        self.delete_button.config(state=tk.DISABLED)

        self.read_button = tk.Button(self.data_frame, text="Actualizar lista", command=self.read_talleres)
        self.read_button.grid(row=10, column=3, pady=10)

        self.selected_taller_id = None  # Inicializar la variable de instancia para el ID
    ...
    def on_double_click(self, event):
        ...
        if selected_item:
            ...
            # Necesitas hacer una consulta para obtener el ID del taller seleccionado
            self.cursor.execute("SELECT IDtaller FROM talleres WHERE Nombre = %s", (values[0],))
            self.selected_taller_id = self.cursor.fetchone()[0]  # Asigna el ID del taller seleccionado
            print(f"Taller seleccionado ID: {self.selected_taller_id}")  # Verifica que sea el ID
    ...
    def confirm_create_taller(self):
        if messagebox.askyesno("Confirmar", "¿Está seguro que desea registrar este taller?"):
            self.create_taller()  # Llama al método original para crear el usuario

    def confirm_update_taller(self):
        if messagebox.askyesno("Confirmar", "¿Está seguro que desea modificar este taller?"):
            self.update_taller()  # Llama al método original para modificar el usuario

    def confirm_delete_taller(self):
        if not self.tree.selection():
            messagebox.showwarning("Advertencia", "Por favor, selecciona un taller para eliminar.")
            return
        if messagebox.askyesno("Confirmar", "¿Está seguro que desea eliminar este taller?"):
            self.delete_taller()  # Llama al método original para eliminar el usuario
     
    def create_taller(self):
        ...
        try:
            self.cursor.execute("""INSERT INTO talleres (Nombre, Tema, Descripcion, Fechaini, Fechafin, Areacomun, Duracion, Requisitos, Instructor, Entidadresponsable) 
                                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)""", 
                                (nombre, tema, descripcion, fechaini, fechafin, areacomun, duracion, requisitos, instructor, entidadresponsable))
            self.conn.commit()

            messagebox.showinfo("Éxito", "Taller registrado exitosamente.")
            self.clear_entries()  # Limpia los campos después de crear la actividad
            self.read_talleres()  # Actualiza la lista de actividades
        except Exception as e:
            messagebox.showerror("Error", f"Error al registrar el taller: {e}")

    def read_talleres(self):
        # Limpiar el Treeview
        for row in self.tree.get_children():
        ...
        try: 
            self.cursor.execute("SELECT IDtaller, Nombre, Tema, Descripcion, Fechaini, Fechafin, Areacomun, Duracion, Requisitos, Instructor, Entidadresponsable FROM talleres")
            ...
            messagebox.showerror("Error", f"Error al leer los talleres: {e}")

        self.adjust_column_widths()  # Ajusta los anchos de las columnas          

    def update_taller(self):
        if self.selected_taller_id is None:
            messagebox.showwarning("Advertencia", "Seleccione un taller para modificar")
            return
        ...
        try:
            self.cursor.execute("""
                ...
                WHERE IDtaller = %s
            """, (nombre, tema, descripcion, fechaini, fechafin, areacomun, duracion, requisitos, instructor, entidadresponsable, self.selected_taller_id))
            
            self.conn.commit()  # Asegúrate de hacer commit

            messagebox.showinfo("Éxito", "Taller actualizado correctamente")
            self.read_talleres()  # Actualizar la lista de actividades
        except Exception as e:
            messagebox.showerror("Error", f"No se pudo actualizar el taller: {e}")
            
    def delete_taller(self):
        if self.selected_taller_id is None:
            messagebox.showwarning("Advertencia", "Seleccione un taller para eliminar")
            return

        # Confirmar la eliminación
        confirm = messagebox.askyesno("Confirmar", "¿Está seguro de que desea eliminar este taller?")
        if confirm:
            try:
                self.cursor.execute("DELETE FROM talleres WHERE IDtaller = %s", (self.selected_taller_id,))
                self.conn.commit()
                messagebox.showinfo("Éxito", "Taller eliminado correctamente")
                self.read_talleres()  # Actualizar la lista de actividades
                self.selected_taller_id = None  # Reiniciar la variable de ID
            except Exception as e:
                messagebox.showerror("Error", f"No se pudo eliminar el taller: {e}")
